...
**Depots** | Pointer to [**[]Depot**](Depot.md) | A list of depots where pickup orders can be delivered to or delivery orders can be picked up from. When providing pickup or delivery orders, at least one depot must be specified. When stopping at a depot, first pickup orders are unloaded.  Afterwards, delivery orders can be loaded into the vehicle. | [optional] [default to []]
