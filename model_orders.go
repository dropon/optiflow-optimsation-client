...
// Orders A list of orders that should be scheduled by the optimization on the routes. If pickup orders or delivery orders are provided, at least one depot must be provided where the pickup orders can be delivered to or the delivery orders can be picked up from. The unique identifiers of the orders must by unique across the three types.
type Orders struct {
